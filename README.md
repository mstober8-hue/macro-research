...
**It has since been strengthened, and it now has a regression form (Part 6).** With occupation and year fixed effects the coefficient is −0.4034 (p = 0.0001) on the 22-25 band used by Brynjolfsson, Chandar and Chen (2026), and −0.4446 (p = 0.0001) for 20-24. It **strengthens** under education and wage controls rather than attenuating (−0.4424 and −0.5095 with both), and the pre-AI placebo on the controlled specification is clean (p = 0.66 for 22-25 and p = 0.45 for 20-24). The published version of this result concedes attenuation on education, pre-existing divergent trends, and weakness in national survey benchmarks. This one improves on all three.
...
| [`splice_diagnostics.py`](splice_diagnostics.py) | Eight BLS/CPS splice variants on the **20-24** 2011-2026 panel; the drift is a denominator artifact and the 2022 break survives 8 of 8. Not used by the paper, which runs on CPS 2016-2026 only |
...
The one result that survived every round of scrutiny, now with a regression form and an external comparison (Part 6). With occupation and year fixed effects across 451 occupations, the young-share coefficient on exposure is −0.4034 (p = 0.0001) on the primary 22-25 band and −0.4446 (p = 0.0001) for 20-24, holds on the raw Eloundou score with no O\*NET term, **strengthens** under education and wage controls, has a clean pre-AI placebo, and survives occupation-specific trends fitted on the pre-period.
