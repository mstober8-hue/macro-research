...
### AI-driven displacement in occupation and sector TOTALS → **NOT SUPPORTED at any level tested.** Totals: null. Entry level: does not survive CPS microdata with occupation fixed effects (all age bands fall together, young share flat). Occupation level: the p = 0.004 result is an artifact of a mis-specified exposure score and two occupations, and every defensible alternative measure flips the sign
...
>
> **Net position: this project has no surviving affirmative evidence of AI labor displacement.** Not in totals, not at the entry level, not at the occupation level. What it has instead is a documented account of why each apparent finding failed, which is a real contribution but a different one. See [`build_cps_panel.py`](build_cps_panel.py) and [`cps_panel_did.py`](cps_panel_did.py).
