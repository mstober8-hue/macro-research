...
**Not everything was wrong, and the distinction is sharp.** Cross-sectional claims, one observation per sector, are valid as computed: nine sectors are nine independent units. Their problem is power, not bias, so they are now reported with a confidence interval and a minimum detectable effect. Time-series claims between persistent series needed the bootstrap.
...
**The out-of-sample validation does not survive.** That matters more than the others, because the out-of-sample replication on 1986-2019 was the reason to believe the rate-hiring lag was not simply fitted to the episode it explains. The in-sample correlation on 2006-2026 is robust, but 2006-2026 *contains* the 2022-2025 episode. So the rate channel is now: strong in the window that includes what it explains, and not statistically distinguishable from noise in the window that does not.
...
### AI-driven displacement in occupation and sector TOTALS → **NOT SUPPORTED in totals. The entry-level version does NOT survive CPS microdata with occupation fixed effects: all age bands fall together and the young share is flat. What survives is that AI-exposed occupations shrank uniformly after 2022 (p = 0.004), which is well-powered but is not age-specific displacement**
...
>
> Every age band falls by nearly the same amount, total occupation employment falls by that same amount again, and the young share does not move. That is AI-exposed occupations **shrinking uniformly**, not displacement concentrated at the entry level. The published-table gradient of −13.1 / −2.8 / −0.0 was an artifact of a lossy title match covering only half of employment.
>
> **What replaces it is a cleaner and better-powered finding.** Exposure predicts occupation employment decline after 2022 at p = 0.004, with 458 clusters, no significant pre-trend in any of the six pre-2022 years, and identification from within-occupation variation. That is a real result. It is a statement about occupations, not about young workers, and it does not distinguish AI from any other force hitting the same occupations. See [`build_cps_panel.py`](build_cps_panel.py) and [`cps_panel_did.py`](cps_panel_did.py).
