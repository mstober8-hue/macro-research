...
**The one substantive positive finding, and it is not displacement.** In the CPS panel, young workers are increasingly concentrated in low-exposure manual and service work. The gap between high- and low-exposure occupations widened from **−5.27pp in 2016 to −7.44pp in 2026**, and it widened because the *low-exposure side moved*: the young share of high-exposure occupations is flat (6.90% to 6.45%) while the young share of low-exposure occupations rose (12.17% to 13.06%). The movement is in where young workers went, not in what they were pushed out of.
...
>
> **Net position: this project has no surviving affirmative evidence of AI labor displacement.** Not in totals, not within industries, not at the occupation level, not at the entry level. What it has instead is a documented account of why each apparent finding failed, and one substantive positive fact: young workers are increasingly concentrated in low-exposure manual and service work, with the gap widening from the low-exposure side rather than from exposed occupations shedding their young. See [`build_cps_panel.py`](build_cps_panel.py), [`cps_panel_did.py`](cps_panel_did.py) and [`entry_level_measure_reaudit.py`](entry_level_measure_reaudit.py).
