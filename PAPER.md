...
**Do the flows contradict the stocks?** The stock results in Section 3.6 carry a
sharp age gradient and the flow results carry none, which looks like a problem
until the implied magnitudes are computed. A stock coefficient of b percentage
points on a band whose base share is s, accumulated over 48 months, implies a
monthly net-flow gap of 100(b/s)/48.

**Table 6.6.** Implied against observed monthly net flow.

| band | stock coef | base share | implied flow | observed flow | 95% CI | agrees |
|---|---:|---:|---:|---:|---:|:--:|
| 20-24 | −0.442 | 9.4% | −0.0978 | −0.1099 | [−0.425, +0.206] | yes |
| **22-25** | **−0.400** | **8.2%** | **−0.1013** | **−0.0922** | [−0.371, +0.187] | **yes** |
| 26-30 | −0.041 | 11.8% | −0.0072 | −0.1355 | [−0.321, +0.050] | yes |
| 31-34 | +0.055 | 9.9% | +0.0115 | +0.0287 | [−0.160, +0.217] | yes |
| 35+ | +0.629 | 62.9% | +0.0208 | −0.0275 | [−0.094, +0.039] | yes |

The implied value lies inside the observed interval at every band, and for the 22
to 25 band the observed estimate is 91 percent of the required magnitude. The
flows agree with the stocks. They are simply too imprecise to confirm the
gradient, and the reason is visible in the table: the 35-and-over band holds 63
percent of employment, so even its large stock coefficient implies a monthly flow
gap of only +0.021. The entire implied gradient spans about 0.12 percentage
points against standard errors of 0.04 to 0.16. Detecting it would require
standard errors roughly three times tighter than monthly CPS transitions support.

This is an underpowered test rather than a conflicting one, and Section 8.7 states
what it does and does not leave open.
...
## 8.7 The flows do not independently confirm the stocks

Section 6.6 shows the flow estimates are consistent with the stock results at
every age band, in the specific sense that the net flow each stock coefficient
implies lies inside the observed interval, and for the primary band the observed
estimate is 91 percent of the implied magnitude. That is reassuring, and it is
weaker than confirmation.

An estimate consistent with a hypothesis is not evidence for it when the interval
is also consistent with zero, and these intervals are. The flow results therefore
neither corroborate nor undermine Section 3. They establish that nothing in the
transition data contradicts it, which is a lower bar.

The margin these flows cannot see is worth naming. A worker who moves from an
exposed occupation to an unexposed one without an intervening spell of
non-employment is counted here as neither a hire nor a separation, and that
movement is precisely the sorting this paper describes. Measuring it requires
occupation-to-occupation transitions among the continuously employed, which the
linked data would support and this paper does not attempt.
